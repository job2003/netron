    ...
    match(context) {
        ...
        if (extension == 'json' && !identifier.endsWith('-symbol.json')) {
            ...
            if (json.indexOf('"mxnet_version":', 0) == -1) {
                try {
                    let root = keras.Utility.parseJson(json);
            ...
            try {
                switch (identifier.split('.').pop().toLowerCase()) {
                    ...
                    case 'model': {
                        ...
                        if (json) {
                            model_config = keras.Utility.parseJson(json);
                        }
                    ...
                    case 'json': {
                        model_config = keras.Utility.parseJson(context.text);
    ...
    static parseJson(json) {
        if (json.indexOf('NaN') !== -1) {
            try {
                ...
                try {
                    return new keras.JsonParser().parse(json);
                }
                catch (err) {
                    // continue regardless of error
                }
